...
void ScriptEditor::keyPressEvent(QKeyEvent *event){
    ...
    if(event->key() == Qt::Key_Control)
        ctlKey = true;
    if(ctlKey && event->key() == Qt::Key_S)
        ((ScriptEditorDialog*)parentWidget())->done(1);
}
...
void ScriptEditor::updateLineNumberArea(QRectF /*rect_f*/){
    updateLineNumberArea();
}

void ScriptEditor::updateLineNumberArea(int /*slider_pos*/){
    updateLineNumberArea();
}

void ScriptEditor::updateLineNumberArea(){
    /*
...
int ScriptEditor::getFirstVisibleBlockId(){
...
void ScriptEditor::lineNumberAreaPaintEvent(QPaintEvent *event){
    ...
    QColor col_1(255, 0, 0);      // Current line (red)
